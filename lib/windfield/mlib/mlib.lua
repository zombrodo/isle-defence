...
local function getCircleCircleIntersection(circle1x, circle1y, radius1, circle2x, circle2y, radius2)
	...
	if checkFuzzy(length, radius1 + radius2) or checkFuzzy(length, math.abs(radius1 - radius2)) then return 'tangent', p3x,
			p3y end
...
local function getPolygonLineIntersection(x1, y1, x2, y2, ...)
	...
	for i = 1, #input, 2 do
		local x1, y1, x2, y2 = getLineSegmentIntersection(input[i], input[i + 1], cycle(input, i + 2), cycle(input, i + 3), x3
			, y3, x4, y4)
...
local function getPolygonSegmentIntersection(x1, y1, x2, y2, ...)
	...
	for i = 1, #input, 2 do
		local x1, y1, x2, y2 = getSegmentSegmentIntersection(input[i], input[i + 1], cycle(input, i + 2), cycle(input, i + 3),
			x1, y1, x2, y2)
		if x1 and not x2 then choices[#choices + 1] = { x1, y1 }
...
local function getPolygonCircleIntersection(x, y, radius, ...)
	...
	for i = 1, #input, 2 do
		local Type, x1, y1, x2, y2 = getCircleSegmentIntersection(x, y, radius, input[i], input[i + 1], cycle(input, i + 2),
			cycle(input, i + 3))
		if x2 then
